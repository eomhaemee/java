...
/**
 *  1을 입력하면 점수 5개의 총점, 평균을 계산하는 프로그램
    2를 입력하면 섭씨를 화씨로 변경하는 프로그램
    3을 입력하면 BMI를 게산하는 프로그램
	...
	public static void main(String[] args){
		...
		while(true){
			...
			if(caseNum ==1){
				...
				String msg = String.format("총점은 %d,평균은 %.2f점입니다.",sum,sum/(double)score.length);//%.2f 소수점 2자리까지만 보여주는 경우
				...
				//BMI지수 = 키m /(체중kg*체중kg)
				String msg = String.format("당신의 키는:  %.1f 체중은: %.2f BMI는 %.2f입니다.", height,weight,(double)weight/(height*height));
				System.out.println(msg);
